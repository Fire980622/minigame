...
namespace UnityEngine.UI
{
    ...
    {
        public static Func<string, UnityEngine.Object> GetSprite;
        public static Func<string, UnityEngine.Object> GetMaterial;
        ...
        //Rotation180顺时针旋转180度
        //Rotation270顺时针旋转270度
        //FlipHorizontal水平翻转
        //FlipVertical垂直翻转
        ...
        [SerializeField]
        private Rotation m_Rotation = Rotation.None;
        public Rotation rotation { get { return m_Rotation; } set { if (SetPropertyUtility.SetStruct(ref m_Rotation, value)) { SetVerticesDirty(); } } }

        /// <summary>
        /// Image绘制范围到RectTransform边缘的padding值
        /// 有利于优化UI相关填充率和图集
        /// </summary>
        [SerializeField]
        private Vector4 m_SpritePadding = Vector4.zero;
        public Vector4 spritePadding { get { return m_SpritePadding; } set { m_SpritePadding = value; } }

        //分离打包中记录的SpriteKey和MaterialKey
        public string spriteKey;
        public string materialKey;


        private void InternalGetSprite()
        {
            if (m_Sprite == null && string.IsNullOrEmpty(spriteKey) == false)
            {
                if (GetSprite != null)
                {
                    m_Sprite = GetSprite(spriteKey) as Sprite;
                }
            }
        }

        private void InternalGetMaterial()
        {
            if (m_Material == null && string.IsNullOrEmpty(materialKey) == false)
            {
                if (GetMaterial != null)
                {
                    m_Material = GetMaterial(materialKey) as Material;
                }
            }
        }
        ...
        static public Material defaultETC1GraphicMaterial
        {
            get
            {
                if (s_ETC1DefaultUI == null)
                    s_ETC1DefaultUI = Resources.Load<Material>("DefaultMaterials/UI-Default-ETC1");
                if (s_ETC1DefaultUI == null)
        ...
        void GenerateSimpleSprite(VertexHelper vh, bool lPreserveAspect)
        {
            ...
            switch (m_Rotation)
            {
                ...
                case Rotation.Rotation90:
                    ...
                    vh.AddVert(new Vector3(v.z, v.y), color32, new Vector2(uv.z, uv.w));
                    break;
                case Rotation.Rotation180:
                    vh.AddVert(new Vector3(v.x, v.y), color32, new Vector2(uv.z, uv.w));
                    vh.AddVert(new Vector3(v.x, v.w), color32, new Vector2(uv.z, uv.y));
                    vh.AddVert(new Vector3(v.z, v.w), color32, new Vector2(uv.x, uv.y));
        ...
        private void GenerateSlicedSprite(VertexHelper toFill)
        {
            ...
            s_VertScratch[0] = new Vector2(padding.x + m_SpritePadding.x, padding.y + m_SpritePadding.y);
            s_VertScratch[3] = new Vector2(rect.width - padding.z - m_SpritePadding.z, rect.height - padding.w - m_SpritePadding.w);
