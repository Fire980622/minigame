...
using System.IO;
    ...
    public class LzmaEncoder : ICoder, ISetCoderProperties, IWriteCoderProperties
    {
        ...
        private const UInt32 kNumLenSpecSymbols = LzmaBase.kNumLowLenSymbols + LzmaBase.kNumMidLenSymbols;
        ...
        private readonly UInt32[] _alignPrices = new UInt32[LzmaBase.kAlignTableSize];
        private readonly UInt32[] _distancesPrices = new UInt32[LzmaBase.kNumFullDistances << LzmaBase.kNumLenToPosStatesBits];

        private readonly BitEncoder[] _isMatch = new BitEncoder[LzmaBase.kNumStates << LzmaBase.kNumPosStatesBitsMax];
        private readonly BitEncoder[] _isRep = new BitEncoder[LzmaBase.kNumStates];
        private readonly BitEncoder[] _isRep0Long = new BitEncoder[LzmaBase.kNumStates << LzmaBase.kNumPosStatesBitsMax];
        private readonly BitEncoder[] _isRepG0 = new BitEncoder[LzmaBase.kNumStates];
        private readonly BitEncoder[] _isRepG1 = new BitEncoder[LzmaBase.kNumStates];
        private readonly BitEncoder[] _isRepG2 = new BitEncoder[LzmaBase.kNumStates];
        ...
        private readonly UInt32[] _matchDistances = new UInt32[LzmaBase.kMatchMaxLen*2 + 2];
        private readonly Optimal[] _optimum = new Optimal[kNumOpts];
        private readonly BitEncoder[] _posEncoders = new BitEncoder[LzmaBase.kNumFullDistances - LzmaBase.kEndPosModelIndex];
        private readonly BitTreeEncoder[] _posSlotEncoder = new BitTreeEncoder[LzmaBase.kNumLenToPosStates];

        private readonly UInt32[] _posSlotPrices = new UInt32[1 << (LzmaBase.kNumPosSlotBits + LzmaBase.kNumLenToPosStatesBits)];
        private readonly RangeCoderEncoder _rangeEncoder = new RangeCoderEncoder();
        private readonly UInt32[] _repDistances = new UInt32[LzmaBase.kNumRepDistances];
        ...
        private readonly UInt32[] repLens = new UInt32[LzmaBase.kNumRepDistances];
        private readonly UInt32[] reps = new UInt32[LzmaBase.kNumRepDistances];
        private readonly UInt32[] tempPrices = new UInt32[LzmaBase.kNumFullDistances];
        ...
        private BitTreeEncoder _posAlignEncoder = new BitTreeEncoder(LzmaBase.kNumAlignBits);
        ...
        private UInt32 GetRepLen1Price(LzmaBase.State state, UInt32 posState)
        {
            return _isRepG0[state.Index].GetPrice0() +
                   _isRep0Long[(state.Index << LzmaBase.kNumPosStatesBitsMax) + posState].GetPrice0();
        }

        private UInt32 GetPureRepPrice(UInt32 repIndex, LzmaBase.State state, UInt32 posState)
        {
            UInt32 price;
            if (repIndex == 0)
            {
        ...
        private UInt32 GetRepPrice(UInt32 repIndex, UInt32 len, LzmaBase.State state, UInt32 posState)
        {
                ...
                if (!nextIsChar && matchByte != currentByte)
                {
                    ...
                    if (lenTest2 >= 2)
                    {
                        ...
                        UInt32 nextRepMatchPrice = curAnd1Price +
                                                   _isMatch[(state2.Index << LzmaBase.kNumPosStatesBitsMax) + posStateNext].
                                                       GetPrice1() +
                            ...
                            if (lenTest < numAvailableBytesFull)
                            {
                                ...
                                if (lenTest2 >= 2)
                                {
                                    ...
                                    UInt32 curAndLenCharPrice = curAndLenPrice +
                                                                _isMatch[
                                                                    (state2.Index << LzmaBase.kNumPosStatesBitsMax) +
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (posSlot >= LzmaBase.kStartPosModelIndex)
                        {
                            ...
                            {
                                _rangeEncoder.EncodeDirectBits(posReduced >> LzmaBase.kNumAlignBits,
                        ...
                        UInt32 distance = pos;
                        for (Int32 i = ((int) LzmaBase.kNumRepDistances) - 1; i >= 1; i--)
                            _repDistances[i] = _repDistances[i - 1];
                        _repDistances[0] = distance;
                        _matchPriceCount++;
                    }
        ...
        private void FillDistancesPrices()
        {
            ...
            for (UInt32 lenToPosState = 0; lenToPosState < LzmaBase.kNumLenToPosStates; lenToPosState++)
            {
                ...
                for (posSlot = LzmaBase.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
                    _posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - LzmaBase.kNumAlignBits) <<
        ...
        {
            private readonly BitTreeEncoder[] _lowCoder = new BitTreeEncoder[LzmaBase.kNumPosStatesEncodingMax];
            private readonly BitTreeEncoder[] _midCoder = new BitTreeEncoder[LzmaBase.kNumPosStatesEncodingMax];
            private BitEncoder _choice;
            private BitEncoder _choice2;
            private BitTreeEncoder _highCoder = new BitTreeEncoder(LzmaBase.kNumHighLenBits);
        ...
        {
            private readonly UInt32[] _counters = new UInt32[LzmaBase.kNumPosStatesEncodingMax];
            private readonly UInt32[] _prices = new UInt32[LzmaBase.kNumLenSymbols << LzmaBase.kNumPosStatesBitsEncodingMax];
