...
using System.IO;
    ...
    public class LzmaDecoder : ICoder, ISetDecoderProperties // ,System.IO.Stream
    {
        ...
        private readonly BitDecoder[] m_IsRep0LongDecoders =
            new BitDecoder[LzmaBase.kNumStates << LzmaBase.kNumPosStatesBitsMax];
        ...
        private readonly LzOutWindow m_OutWindow = new LzOutWindow();
        ...
        private readonly RangeCoderDecoder m_RangeDecoder = new RangeCoderDecoder();
        private readonly LenDecoder m_RepLenDecoder = new LenDecoder();
        private bool _solid;

        private uint m_DictionarySize;
        private uint m_DictionarySizeCheck;
        private BitTreeDecoder m_PosAlignDecoder = new BitTreeDecoder(LzmaBase.kNumAlignBits);
                         ...
                         Int64 inSize, Int64 outSize, ICodeProgress progress)
        {
            ...
            while (nowPos64 < outSize64)
            {
                ...
                // while(nowPos64 < next)
                {
                    ...
                    if (
                        m_IsMatchDecoders[(state.Index << LzmaBase.kNumPosStatesBitsMax) + posState].Decode(m_RangeDecoder) ==
                        0)
                    {
                        byte b;
                    ...
                    else
                    {
                        ...
                        if (m_IsRepDecoders[state.Index].Decode(m_RangeDecoder) == 1)
                        {
                            ...
                            {
                                if (
                                    m_IsRep0LongDecoders[(state.Index << LzmaBase.kNumPosStatesBitsMax) + posState].Decode(
                                        m_RangeDecoder) == 0)
                                {
        ...
        {
            private readonly BitTreeDecoder[] m_LowCoder = new BitTreeDecoder[LzmaBase.kNumPosStatesMax];
            private readonly BitTreeDecoder[] m_MidCoder = new BitTreeDecoder[LzmaBase.kNumPosStatesMax];
            private BitDecoder m_Choice;
            private BitDecoder m_Choice2;
            private BitTreeDecoder m_HighCoder = new BitTreeDecoder(LzmaBase.kNumHighLenBits);
