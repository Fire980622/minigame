...
-- PS：仙侠这边的TimerManager有用update重写。其他项目如果有必要也可以实现一个，替换LuaTimner.Add
-- ====================================================================
CombatTimer = CombatTimer or BaseClass()

-- 单例
function CombatTimer:__init()
    if CombatTimer.Instance then
        Log.Error("不可以对单例[CombatTimer]对象重复实例化")
        return
    end
    self.all_timer = {}
    self.auto_id = 0
...
function CombatTimer:Add(obj, delay_ms, callback_func, timer_name)
    ...
    local time_id = TimerManager.Add(delay_ms, function() self:__callback__(obj, timer_name, callback_func) end)
...
function CombatTimer:Delete(timer_name)
    if self.all_timer[timer_name] then
        TimerManager.Delete(self.all_timer[timer_name])
...
function CombatTimer:__run_one_timer__(timer_obj, args, time_id)
    if timer_obj then
        ...
        elseif obj_type == CombatTimer.Type.SEQUENCE then
            ...
            Log.Error(string.format("不存在ObjTimer定时器类型[%s]", obj_type))
        end
    end
end
...
function CombatTimer:__add__(obj, delay_ms, callback_func, timer_name)
    ...
    local timer_id = TimerManager.Add(delay_ms, function() self:__callback_mix__(obj, timer_name, callback_func) end)
    timer_name = timer_name or self:__auto_id__()
