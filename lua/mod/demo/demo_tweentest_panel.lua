...
DemoTweenTestPanel = DemoTweenTestPanel or BaseClass(BasePanel)

function DemoTweenTestPanel:__init(model, parent)
    self.name = "DemoTweenTestPanel"
    self.model = model
    self.parent = parent
    self.resList = {
        {path = AssetConfig.uitween_panel_prefab, type = AssetType.Prefab}
    }
...
function DemoTweenTestPanel:__delete()
    if self.previewComp1 ~= nil then
        self.previewComp1:DeleteMe()
    end
end

function DemoTweenTestPanel:InitPanel()
    self.gameObject = self:GetGameObject(AssetConfig.uitween_panel_prefab)
...
function DemoTweenTestPanel:LoadPreview()
...
function DemoTweenTestPanel:PreviewLoaded(composite)
...
function DemoTweenTestPanel:DoHorizontal()
    if self.rawImage ~= nil then
...
function DemoTweenTestPanel:DoRotate()
    if self.rawImage ~= nil then
        if self.modeltween ~= nil then
            Tween.Instance:Cancel(self.modeltween.id)
            self.modeltween = nil
...
function DemoTweenTestPanel:DoScale()
    if self.rawImage ~= nil then
        if self.modeltween ~= nil then
            Tween.Instance:Cancel(self.modeltween.id)
            self.modeltween = nil
