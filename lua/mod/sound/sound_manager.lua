...
function SoundManager:InitPlayer()
    self.audioListener = GameObject.Find("MainCamera"):AddComponent(typeof(AudioListener))
...
function SoundManager:PlayBGM(soundId)
    ...
    local callback = function(soundId, clip, soundPath)
        self.bgmLoading = false
        self:OnBgmLoadCompleted(soundId, clip, soundPath)
    end
...
function SoundManager:OnBgmLoadCompleted(soundId, clip, soundPath)
    local player = self.playerList[AudioSourceType.BGM]
    if self.bgmPlayingPath ~= nil then
        AssetMgrProxy.Instance:DecreaseReferenceCount(self.bgmPlayingPath)
        self.bgmPlayingPath = nil
    end
    self.bgmPlayingPath = soundPath
    AssetMgrProxy.Instance:IncreaseReferenceCount(self.bgmPlayingPath)
    player:SetClip(clip, soundPath)
    player:Play()
end
...
function SoundManager:Play(soundId)
    ...
    local clip = PreloadManager.Instance:GetEffectSoundClip(tostring(soundId))
    player:SetClip(clip)
    player:Play()
end
...
function SoundManager:PlayCombat(soundId)
...
function SoundManager:PlayCombatChat(soundId)
...
function SoundManager:PlayCombatHiter(soundId)
...
function SoundManager:PlayCombatHit(soundId)
...
function SoundManager:PlayChat(clip)
