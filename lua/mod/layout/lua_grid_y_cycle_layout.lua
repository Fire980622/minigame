...
e.g
    ...
    local function GridCellAtIndex(cloneCell, index)
        cloneCell.transform:Find("Text"):GetComponent(typeof(Text)).text = index
    end
...
LuaGridCycleLayout = LuaGridCycleLayout or BaseClass()
...
function LuaGridCycleLayout:__init(panel, setting)
    ...
    self.scrollRect:GetComponent(typeof(ScrollRect)).onValueChanged:AddListener(self.funcScroll)
...
function LuaGridCycleLayout:DeActivated()
    self.scrollRect:GetComponent(typeof(ScrollRect)).onValueChanged:RemoveListener(self.funcScroll)
end
...
function LuaGridCycleLayout:AddCell(cell)
    ...
    local cellData = {cellTran = cell:GetComponent(typeof(RectTransform)), cell = cell, pos = pos}
...
function LuaGridCycleLayout:GetCacheCellByIndex(index)
	...
	if not self.cacheList[cacheIndex] then
	    ...
	    local rect = newCell:GetComponent(typeof(RectTransform))
...
function LuaGridCycleLayout:ReloadData()
    ...
    for k, v in ipairs(self.cacheList) do
        v:GetComponent(typeof(RectTransform)).anchoredPosition3D = self.unvisibleVec
    end
