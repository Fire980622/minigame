...
function TabbedPanel:__init(panel, pageCount, perWidth, tweenTime)
    ...
    self.scrollbar = panel.transform:GetComponent(typeof(ScrollRect))
    self.scrollbar.inertia = false
    self.scrollbar.elasticity = 0
    -- if scrollbar ~= nil then
    --     self.scrollbar = scrollbar:GetComponent(typeof(Scrollbar))
    -- end
    self.rectTransform = self.container:GetComponent(typeof(RectTransform))
    ...
    self.pageController  = self.panel:AddComponent(typeof(PageTabbedController))
