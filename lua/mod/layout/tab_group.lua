...
function TabGroup:__init(gameObject, callback, setting)
    ...
    self.rect = self.gameObject:GetComponent(typeof(RectTransform))
...
function TabGroup:Init()
    ...
    for i = 1, childcount do
        ...
        obj:GetComponent(typeof(Button)).onClick:AddListener(function() self:ChangeTab(index) end)
        local transform = obj.transform
        tab["gameObject"] = obj
        tab["transform"] = transform
        tab["rect"] = obj:GetComponent(typeof(RectTransform))
        ...
        if red_transform ~= nil then tab["red"] = red_transform.gameObject end
        local text_transform = transform:Find("Text")
        if text_transform ~= nil then tab["text"] = text_transform:GetComponent(typeof(Text)) end
