...
LuaBoxLayout = LuaBoxLayout or BaseClass()
...
function LuaBoxLayout:__init(panel, setting)
    ...
    self.panel = panel
    self.panelRect = self.panel:GetComponent(typeof(RectTransform))
    self.cellList = {}
    self.contentSize = 0
    ...
    if self.scrollRect ~= nil then
        ...
        self.scrollRect:GetComponent(typeof(ScrollRect)).onValueChanged:AddListener(
            function(value)
...
function LuaBoxLayout:AddCell(cell)
    local rect = cell:GetComponent(typeof(RectTransform))
...
function LuaBoxLayout:SetPosition(rect, index)
    ...
    if self.axis == BoxLayoutAxis.X then
        ...
        rect.anchoredPosition3D = Vector2(self.contentSize + self.spacing, 0, 0)
        ...
        self.panelRect:SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, self.contentSize)
    else
        ...
        rect.anchoredPosition3D = Vector2(x, y, 0)
        self.contentSize = self.contentSize + cellSize + self.border
        if self.Top then
            self.contentSize = self.contentSize + self.Top
        end
        self.panelRect:SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, self.contentSize)
    end
end
    ...
    if self.axis == BoxLayoutAxis.X then
        ...
        if self.Dir == BoxLayoutDir.Left then
            ...
            for i,child in ipairs(self.cellList) do
                local show = child.transform.anchoredPosition.x+child.transform.sizeDelta.x > Left and child.transform.anchoredPosition.x < Right
                child:SetActive(show)
            end
        else
            local Left = (contentSize - scrollSizeDelta.x) * (value.x)
            local Right = scrollSizeDelta.x + Left
            if Right > contentSize + self.spacing then
                Right = contentSize + self.spacing
            end
        ...
        if self.Dir == BoxLayoutDir.Top then
            ...
            for i,child in ipairs(self.cellList) do
                local show = child.transform.anchoredPosition.y < top and child.transform.anchoredPosition.y+child.transform.sizeDelta.y > bot
                child:SetActive(show)
            end
        else
            for i,child in ipairs(self.cellList) do
                local show = -child.transform.anchoredPosition.y+child.transform.sizeDelta.y > top and -child.transform.anchoredPosition.y < bot
                child:SetActive(show)
            end
        end
    end
