...
function LuaCycleList.refresh_circular_list(setting_data)
    ...
    if setting_data.data_list == nil then
        setting_data.item_con:GetComponent(typeof(RectTransform)).sizeDelta = Vector2(0, 0)
        return
    end
    if #setting_data.data_list == 0 then
        ...
        setting_data.item_con:GetComponent(typeof(RectTransform)).sizeDelta = Vector2(0, newH)
    end
    setting_data.item_con:GetComponent(typeof(RectTransform)).anchoredPosition = Vector2(0, 0)

    for i=1,#setting_data.item_list do
        local newY = (i-1)*-setting_data.single_item_height
        local _X = setting_data.item_list[i].gameObject.transform:GetComponent(typeof(RectTransform)).anchoredPosition.x
        setting_data.item_list[i].transform:GetComponent(typeof(RectTransform)).anchoredPosition = Vector2(_X, newY)
    end
...
function LuaCycleList.static_refresh_circular_list(setting_data)
    if #setting_data.data_list < #setting_data.item_list then
        ...
        if offset_index ~= 0 then
            for i=1,#setting_data.item_list do
                local item = setting_data.item_list[i]
                local oldY = item.transform:GetComponent(typeof(RectTransform)).anchoredPosition.y
                local offset_y = offset_index*setting_data.single_item_height
                local newY = oldY + offset_y
                local _X = item.gameObject.transform:GetComponent(typeof(RectTransform)).anchoredPosition.x
                item.transform:GetComponent(typeof(RectTransform)).anchoredPosition = Vector2(_X, newY)
            end
        end
...
function LuaCycleList.on_value_change(setting_data)
    --核心逻辑
    local cur_y = setting_data.item_con:GetComponent(typeof(RectTransform)).anchoredPosition.y
    ...
    for i=1,cross_step do
        if cur_y <= setting_data.item_con_last_y then
            if setting_data.data_head_index ~= 1 and cur_y < (setting_data.item_con_height - setting_data.scroll_con_height) then
                ...
                local oldY = tail_item.transform:GetComponent(typeof(RectTransform)).anchoredPosition.y
                if math.abs(oldY) > (math.abs(cur_y)+setting_data.scroll_con_height+2*setting_data.single_item_height) then
                    ...
                    local _X = tail_item.gameObject.transform:GetComponent(typeof(RectTransform)).anchoredPosition.x
                    tail_item.transform:GetComponent(typeof(RectTransform)).anchoredPosition = Vector2(_X, newY)
        ...
        elseif cur_y > setting_data.item_con_last_y then
            if setting_data.data_tail_index ~= #setting_data.data_list and cur_y >= setting_data.single_item_height then
                ...
                local oldY = head_item.transform:GetComponent(typeof(RectTransform)).anchoredPosition.y
                if math.abs(oldY) < math.abs(cur_y) - 2*setting_data.single_item_height then
                    ...
                    local newY = oldY - #setting_data.item_list*setting_data.single_item_height
                    local _X = head_item.gameObject.transform:GetComponent(typeof(RectTransform)).anchoredPosition.x
                    head_item.transform:GetComponent(typeof(RectTransform)).anchoredPosition = Vector2(_X, newY)
