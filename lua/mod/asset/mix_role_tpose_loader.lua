...
-- @ljh
MixRoleTposeLoader = MixRoleTposeLoader or BaseClass()
...
function MixRoleTposeLoader:BuildWeaponTpose(result)
    if not self.cancel then
        ...
        local t = self.weapon:GetComponent(typeof(Transform))
        ...
        if self.weapon2 ~= nil then
            ...
            local t2 = self.weapon2:GetComponent(typeof(Transform))
