...
-- @author huangyq
AssetDuplicateLoader = AssetDuplicateLoader or BaseClass()
    ...
    if #self.resPList > 0 then
        ...
        for _, data in ipairs(pCell.list) do
            ...
            elseif data.type == AssetType.Asset then
                ...
                Log.Error("【非法资源类型】 path:" .. tostring(data.path) .. "  type:" .. tostring(data.type))
            end
        end
    end
end
...
function AssetDuplicateLoader:GetEvent(priority)
    if priority == nil then
        Log.Error("call GetEvent Error: priority is nil")
    end
